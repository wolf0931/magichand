<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button :disabled="selectedIds.length === 0" class="filter-item" type="danger" icon="el-icon-delete" @click="handleBatchDelete">批量删除</el-button>
    </div>
    <el-table
    ...
    >
      ...
      <el-table-column type="expand" fixed="left" label="sku信息">
      ...
      <el-table-column align="center" fixed="right" label="操作" width="250" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button  v-if=" scope.row.status == 1" type="warning" size="mini" @click="freezeOrActivationBtn(scope.row,'freeze')">下架</el-button>
          <el-button  v-else type="success" size="mini" @click="freezeOrActivationBtn(scope.row,'activation')">上架</el-button>
          <el-button  type="danger" style="margin:5px" size="mini" @click="handleDelete(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <pagination
...
<script>
import { listGoods,deleteGood} from '@/api/mall/goods'
import {categoryTree} from '@/api/mall/category'
